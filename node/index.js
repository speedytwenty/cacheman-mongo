...
var createIndexes = function createIndexes(collection, cb) {
  return Promise.all([collection.createIndex({ key: 1 }, { unique: true }), collection.createIndex({ expireAt: 1 }, { expireAfterSeconds: 0 })]).then(function () {
    ...
    this.ready = (0, _thunky2.default)(function (cb) {
      if (!_this.collection) {
        ...
        _mongodb.MongoClient.connect(_this.url, { useUnifiedTopology: true }, function (err, client) {
          ...
          createIndexes(_this.collection, cb);
        });
      } else {
        if (_this.client) return createIndexes(_this.collection, cb);
